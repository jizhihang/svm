...
if ~isstr(Xt),
  ...
  if model.x_dim==1,
    ...
    ylabel('Y');
    [~,si] = sort(X);
    plot(X(si),Yt(si),'Color',colorMat(1,:)); hold on; 
    plot(X(si),Yt(si)+2.*sige(si),'-.','Color',colorMat(2,:));    
    plot(X(si),Yt(si)-2.*sige(si),'-.','Color',colorMat(2,:));
    plot(X(si),Y(si),'o','Color',colorMat(4,:)); hold off;
  else
    xlabel('time');
    ylabel('Y');
    plot(Yt,'Color',colorMat(1,:)); hold on; 
    plot(Yt+2.*sige,'-.','Color',colorMat(2,:)); 
    plot(Yt-2.*sige,'-.','Color',colorMat(2,:));
    plot(Y,'o','Color',colorMat(4,:)); hold off;
  end
